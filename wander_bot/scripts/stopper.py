...
class SnakeRobot:
	...
	def check_is_wall(self):
		...
		minIndex = round((self.MIN_SCAN_ANGLE - self.laser.angle_min) / self.laser.angle_increment)
		maxIndex = round((self.MAX_SCAN_ANGLE - self.laser.angle_min) / self.laser.angle_increment)
		# print(minIndex, maxIndex, self.MIN_SCAN_ANGLE, self.MAX_SCAN_ANGLE, msg.angle_min, msg.angle_max)
		# print(len(msg.ranges))

		currIndex = minIndex + 1
		while currIndex <= maxIndex:
			if self.laser.ranges[int(currIndex)] < self.MIN_DIST_FROM_OBSTACLE:
	...
	def rotate180(self):
		...
		if (self.state.state == RobotStatus.turn_left):
			turn_angle = math.radians(-180)
		
		if (self.state.state == RobotStatus.turn_right):
			turn_angle = math.radians(180)
	...
	def rotate(self):
		...
		print(self.state.state)
		while rospy.Time.now() < end:
			twist.angular.z = direction * self.ROTATE_SPEED
			self.pub.publish(twist)
		print("Stop Rotating")
		twist.angular.z = 0
		self.pub.publish(twist)

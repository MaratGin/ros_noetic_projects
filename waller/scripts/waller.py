...
class WallerRobot:
	...
	def callback(self, msg):
		...
		if self.data.state == RobotState.find_wall:
			...
			left_front = self.get_front_left(msg)
			orientation = left_front / right_avg
            ...
            # angular velocity using PD control
				twist.angular.z = 1.5*max_angular * ((self.Kp * error) + self.Kd * (error - self.error_prev)/0.05)
        ...
        # if robot is pointed towards the wall
            ...
            # angular velocity using PD control
				twist.linear.x = 0
				twist.angular.z = 4.5*max_angular * ((self.Kp * error) +self.Kd * (error - self.error_prev)/0.05)
			else:
				print("ERROR")
			self.pub.publish(twist)
			# rate.sleep()
